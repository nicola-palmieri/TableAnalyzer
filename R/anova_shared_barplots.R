...
prepare_nested_significance_annotations_data <- function(stats_df,
  ...
  base_df <- tidyr::expand_grid(
    !!sym(factor1) := factor(lev1, levels = lev1),
    !!sym(factor2) := factor(lev2, levels = lev2)
  )

  base_df$y_position <- as.numeric(mapply(
    height_lookup,
    xlvl = base_df[[factor1]],
    glvl = base_df[[factor2]]
  )) + offset
  base_df$annotations <- ""
  ...
  for (g1 in lev1) {
    for (lvl in lev2[lev2 != reference_level]) {
      ...
      idx <- base_df[[factor1]] == g1 & base_df[[factor2]] == lvl
      base_df$annotations[idx] <- annotation
      base_df$y_position[idx] <- y_base + offset
    }
  }

  annotated_df <- dplyr::filter(base_df, .data$annotations != "")
  if (nrow(annotated_df) == 0) return(NULL)

  list(
    data = base_df,
    max_y = max(annotated_df$y_position, na.rm = TRUE) * 1.05
  )
}
